  ...
  <div v-loading="loading && !data" class="platform-ai-usage">
    ...
    <section class="platform-ai-usage__panel">
      ...
      <el-table :data="runRows" stripe class="hfl-list-table" row-key="run_uuid">
        <el-table-column label="Time" width="170"><template #default="{ row }"><span :class="{ 'hfl-empty-mark': !row.time }">{{ formatLocalDateTime(row.time, '—') }}</span></template></el-table-column>
        <el-table-column label="Request" min-width="280"><template #default="{ row }"><button type="button" class="platform-ai-usage__request-button" @click="openRun(row)">{{ row.question || row.chat_title || 'Untitled request' }}</button><span class="platform-ai-usage__cell-meta" :class="{ 'hfl-empty-mark': !row.backup_source_name && !row.chat_title }">{{ row.backup_source_name || row.chat_title || '—' }}</span></template></el-table-column>
