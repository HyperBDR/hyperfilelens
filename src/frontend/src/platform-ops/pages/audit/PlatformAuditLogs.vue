    ...
    <div class="platform-monitoring-page">
      ...
      <HflTablePanel fill>
        ...
        <template #table="{ tableMaxHeight }"><el-table v-loading="loading" :data="rows" stripe flexible row-key="id" class="hfl-list-table" :max-height="tableMaxHeight">
          <el-table-column label="Time" width="175"><template #default="{ row }"><span :class="{ 'hfl-empty-mark': !row.created_at }">{{ formatLocalDateTime(row.created_at, '—') }}</span></template></el-table-column>
